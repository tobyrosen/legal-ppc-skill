...
- `campaign.geo_target_type_setting.positive_geo_target_type`: account `PRESENCE` (baseline `PRESENCE_OR_INTEREST`), scope: Search - Family Law, approved by toby (tg-1234) on YYYY-MM-DD, entry `example-family-law-YYYYMMDD-NN`
...
- `campaign_budget.explicitly_shared`: account `true` (baseline `false`), scope: the two custody campaigns, approved by toby (tg-1235) on YYYY-MM-DD, entry `example-family-law-YYYYMMDD-NN`
