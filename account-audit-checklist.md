...
- [ ] No deprecated tracking sources present (e.g., the call-tracking vendor — acquired, unreliable)
- [ ] GA4 and HubSpot (or other platforms) are not double-counting the same form submissions
- [ ] `counting_type = MANY_PER_CLICK` on lead actions (agency standard: every touch is signal for data-driven attribution; see agency-defaults §3.4)
